...
#
# Поднимает mock-Firebase, собирает игру и гоняет два сценария:
#   1. два реальных клиента находят друг друга и видят состояние соперника;
#   2. поддельный соперник расстреливает клиента, у того падает ХП.
set -u
...
echo
echo "== сценарий 1: два живых клиента =="
...
echo
echo "== сценарий 2: урон от чужой пули =="
curl -s -X DELETE "$BASE/rooms/main.json" >/dev/null
