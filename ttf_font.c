...
DSFont *ds_font_create(const uint8_t *data, size_t size, int ph) {
    ...
    if (!add_utf8(f,
        ...
        /* Символы, которых раньше не было в атласе: игра рисовала вместо них
         * «?» (например, «Buy · 100 Candies» и половина японских строк).
         * Список проверяется тестом test/check_font_coverage.py. */
        "·«»×"
        "登録文字正読込試合回復活杖強氷吸収足近公開買")) {
